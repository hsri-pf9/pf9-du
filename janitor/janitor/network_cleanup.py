...
class NetworkCleanup(Base):
    ...
    def build_instance_network_mapping(self,  token, project_id, network_ids):
        """
        Figure out which interfaces are connected to networks about to be removed.
        :param token:
        :param project_id:
        :param network_ids: Dangling networks to be removed
        :return:
        """
        resp = self._nova_request('servers?all_tenants=1', token, project_id)
        if resp.status_code != requests.codes.ok:
            LOG.error('Received response: {code}, skipping network cleanup'.format(code=resp.status_code))
            raise InvalidResponse('No virtual machines found, invalid response {code}'.format(code=resp.status_code))
        ...
        for instance_id in instance_ids:
            ...
            if 'virtual_interfaces' in resp.json():
                ...
                for vif_info in vif_infos:
                    if 'OS-EXT-VIF-NET:net_id' in vif_info and vif_info['OS-EXT-VIF-NET:net_id'] in network_to_inst:
                        network_to_inst[vif_info['OS-EXT-VIF-NET:net_id']].append((instance_id, vif_info['id']))
    ...
    def cleanup(self):
        ...
        for n in dangling_networks:
            ...
            for inst_id, vif_id in net_to_inst[n]:
                ...
                resp = self._nova_request('servers/{srv_id}/action'.format(srv_id=inst_id), token,
                                          project_id, req_type='post', json_body=body)
                if resp.status_code not in (requests.codes.ok, 202):
                    LOG.error('Error cleaning up VIF record {vif} from instance {inst}, status: {stat}'
            ...
            if resp.status_code not in (requests.codes.ok, 204, 202):
                LOG.error('Skipping network deletion, return code {code}'.format(code=resp.status_code))
                continue
