...
def clean_packages(log):
    ...
    if os.path.isfile(HOSTID_CONF):
        ...
        try:
            ...
            log.exception("Error parsing host id from {0}".format(HOSTID_CONF))
        if os.path.isfile(desired_apps) and os.path.isdir('/var/cache/pf9apps'):
            try:
                json_file = open(desired_apps)
                data = json.load(json_file)
            except Exception:
                log.exception("Error reading contents from {0}".format(desired_apps))
            json_file.close()
            for package in data:
                ...
                for dir in os.listdir(pkg_cache_dir):
                    if version != dir:
                        ...
                        try:
                            ...
                            log.exception("Error removing the cached package file")
